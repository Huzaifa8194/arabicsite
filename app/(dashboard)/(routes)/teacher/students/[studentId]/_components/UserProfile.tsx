...
const UserProfile = async ({ params }: PageProps) => {
  ...
  const courserWithProgress: any = await getCourses({
  ...
  const userMessagesSorted = userMessages.sort(
    (a: any, b: any) => b.createdAt - a.createdAt
  );
  const lastMessage =
    userMessagesSorted.length > 0 ? userMessagesSorted[0] : null;
  return (
    <div>
          ...
          <div className="bg-white shadow-2xl rounded-lg py-3 px-3">
            ...
            <div className="p-2">
              <h3 className="text-center text-xl text-gray-900 font-medium leading-8">{`${
                userInfo.firstName
              } ${userInfo.lastName ? userInfo.lastName : ""}`}</h3>
              <div className="text-center text-gray-400 text-xs font-semibold">
                <p>
                  اخر تسجيل دخول:
                  {`${new Date(userInfo.updatedAt).getDate()}/${
                    new Date(userInfo.updatedAt).getMonth() + 1
                  }/${new Date(userInfo.updatedAt).getFullYear()}`}
                </p>
                <p>
                  اخر رسالة :
                  {lastMessage
                    ? `${new Date(
                      ...
                      ` - ` +
                      `${new Date(lastMessage?.updatedAt!).getDate()}/${
                        new Date(lastMessage?.updatedAt!).getMonth() + 1
                      }/${new Date(lastMessage?.updatedAt!).getFullYear()}`
                    : `لم يرسل اي رسالة`}
                </p>
              </div>
                ...
                <tbody>
                  ...
                  {courserWithProgress.map(
                    async (
                      ...
                      for (let i = 0; i < chapters.length; i++) {
                        ...
                        for (let j = 0; j < lessons.length; j++) {
                          ...
                          const isLessonCompleted =
                            await db.userProgress.findFirst({
                              where: {
                                lessonId: lesson.id,
                                userId: params.studentId,
                              },
                            });
                          let lessonTime = Math.floor(
                            ((Date.parse(`${isLessonCompleted?.updatedAt}`) -
                              isLessonCompleted?.startedAt!) /
                              1000) %
                              60
                          );
                          time =
                        ...
                        for (let k = 0; k < quizs.length; k++) {
                          ...
                          const isQuizCompleted =
                            await db.userQuizPoints.findFirst({
                              where: {
                                quizId: quiz.id,
                                userId: params.studentId,
                              },
                      ...
                      const startCourse = exams.find((e: any) => {
                        return e.starterExam;
                      });
                      const finalCourse = exams.find((e: any) => {
                        return !e.starterExam;
                      });
                      ...
                      if (
                        ...
                        return (
                          <UserStats
                            ...
                            course={course}
                            key={index}
                            quizsCompleted={quizsCompleted}
                            lessonsCompleted={lessonsCompleted}
                            timeString={timeString}
                            firstExamsPrgress={firstExamsPrgress?.percentage!}
                            finalExamsPrgress={finalExamsPrgress?.percentage!}
                          />
                        );
                      }
                    }
